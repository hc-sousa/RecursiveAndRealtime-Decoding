...
static void recursive_decoder(int encoded_idx,int decoded_idx,int good_decoded_size)
{
  ...
  for (int i = encoded_idx; i < _max_encoded_message_size_ && i-encoded_idx+1 <= _c_->max_bits; i++)
  {
    ...
    for (int j = 0; j < _c_->n_symbols ; j++)
    {
      if (strcmp(_c_->data[j].codeword, tempMessage) == 0)
      {
        ...
        if (decoded_idx == good_decoded_size && _decoded_message_[decoded_idx] == _original_message_[decoded_idx])

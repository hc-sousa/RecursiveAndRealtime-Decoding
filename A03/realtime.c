...
long maxPossibilities, nPossibilities;
...
//
  ...
  // encoded_idx somar o numero de bits q foram descodificados
  // decoded_idx somar um por cada indice descodificado
  /* good_decoded_size por cada simbolo descodificado
  se o decoded idx for igual a good decoded size
  se o simbolo bater certo ao simbolo da mensagem original
  good decoded + 1 e chama recursivamento
  se good decode < decoded idx
  chamar recursivamente sem alterar o good decode
  */
/*
static long realtime_decoder()
{
  long potencia = pow(2.0,(double) _c_->n_symbols);
  char tempDecode[potencia], newDecode[potencia], string[_max_decoded_message_size_];
  int encoded_idx = 0, decoded_idx = 0;
  while (decoded_idx < _original_message_size_)
  {
    char tempMessage[encoded_idx+2], tempSymb[encoded_idx+2]; int n = -1, c = -1, h = 0;
    while (++n <= encoded_idx)
    ...
    for (int j = 0; j < _c_->n_symbols ; j++){
      ...
      tempSymb[n] = '\0';
      //printf("%s = %s = %s\n", tempSymb, _c_->data[j].codeword, tempMessage);
      n = -1;
...
bool verifySequence(char* decodedSequence, char* tempEncoded, int decoded_idx, int encoded_idx){
  char encoded[_max_encoded_message_size_]; int n = 0;
  for (int i = 0; i <= decoded_idx+1; i++){
    for (int j = 0; j < _c_->n_symbols ; j++){
      if (strcmp(decodedSequence[i], _c_->data[j].codeword) == 0)
        encoded[n++] = _c_->data[j].codeword;
    }
  }
